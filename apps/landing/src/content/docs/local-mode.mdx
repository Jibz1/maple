...
The installer detects your OS/arch, downloads the matching bundle from the latest GitHub release, verifies its checksum, installs the two files (`maple` + `libchdb`) into `~/.maple/bin`, clears the macOS Gatekeeper quarantine, and symlinks `maple` onto your `PATH`. macOS (Apple Silicon & Intel) and Linux (x86_64 & arm64) are supported.
